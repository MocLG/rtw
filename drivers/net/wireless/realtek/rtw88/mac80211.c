 ...
 * connect window, matching the vendor STA path.
 */
...
static bool rtw8723bs_mgd_prepare_is_auth(struct ieee80211_prep_tx_info *info)
{
	return info && info->subtype == IEEE80211_STYPE_AUTH;
}
				       ...
				       const u8 *bssid)
{
	...
	rtw8723bs_auth_rx_filter(rtwdev);
...
/*
 * chip_ops::prepare_tx for the RTL8723BS: orchestrate the pre-auth join by
 * programming the vendor join state, sending the pre-auth deauth and then
 * replaying the pre-auth coex H2Cs, once per fresh BSSID.
 */
			  ...
			  struct ieee80211_prep_tx_info *info)
{
	...
	/*
	 * Wake from soft IPS so the register writes below reach the chip. The
	 * RFK is handled by the once-only power-on IQK plus the ps.c post-IPS
	 * RF bus recovery, not a fresh calibration here.
	 */
	...
	if (!rtw8723bs_mgd_prepare_is_auth(info) || !vif ||
...
EXPORT_SYMBOL(rtw8723bs_prepare_tx);
